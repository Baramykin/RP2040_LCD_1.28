...
def character(asc,xt,yt,sz,r,g,b):  # Single character sz is size: 1 or 2
    cc = colour(r,g,b)
    code = asc * FONT_WIDTH    # 5 bytes per character
    for ii in range(FONT_WIDTH):  # 5
        line = FONT[code + ii]
        for yy in range(FONT_HEIGHT): # 8
            if (line >> yy) & 0x1:
...
def n_sign():
    ...
    for asd in range(1, lenght+1, 1):
        lines_1 =read_file(asd, "example.txt")  # example.txt output.txt
        ca = colour(255,0,0)
        for col in range(68):
            LCD.pixel(col+start_point_n_x,start_point_n_y,int(lines_1[col]))
        start_point_n_y += 1
...
while True:   
    ...
    # n_sign()
    # ca = colour(255,0,0)
    # for yy in range(start_point_n_y,start_point_n_y+48,1):
    #     for xx in range(start_point_n_x,start_point_n_x+68,1):
    #         a = ca
            # LCD.pixel(xx,yy,ca)
        # Write Title and Scale values
    cntr_st("BV   DEV",30,2,255,255,255) # Message
